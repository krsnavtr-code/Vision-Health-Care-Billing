...
export default function AdminBillsOverview() {
  ...
  const fetchUserBills = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await apiCall("/invoices/admin/overview");
      if (res.success) {
        setUserBills(res.data);
      }
    } catch (err) {
      setError(err.message || "Failed to fetch bills overview");
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleUserExpand = (patientId) => {
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-black text-slate-800 tracking-tight">
              Bills Overview by User
            </h1>
            <p className="text-xs text-slate-500 mt-1">
              View all generated bills grouped by patient
            </p>
          </div>
          <button
      ...
      {!loading && userBills.length > 0 && (
        <div className="max-w-7xl mx-auto space-y-3">
          {userBills.map((userBill) => {
            const isExpanded = expandedUsers[userBill.patient._id];
            return (
              <div
                key={userBill.patient._id}
                className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden"
              >
                {/* User Header */}
                <div
                  className="p-4 flex items-center justify-between cursor-pointer hover:bg-slate-50 transition"
                  onClick={() => toggleUserExpand(userBill.patient._id)}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-slate-800">
                        {userBill.patient.name}
                      </h3>
                      <p className="text-xs text-slate-500 mt-0.5">
                        {userBill.patient.phoneNumber || "No phone"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-black text-emerald-600">
                        ₹{userBill.totalAmount.toFixed(2)}
                      </p>
                    </div>
                    {isExpanded ? (
